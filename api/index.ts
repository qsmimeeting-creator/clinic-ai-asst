...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    for (const apiKey of shuffledKeys) {
      try {
        ...
        const responseStream = await ai.models.generateContentStream({
          ...
          config: {
            systemInstruction: `คุณคือผู้ช่วย AI ของคลินิก กฎเหล็กสูงสุดของคุณคือ: "ต้องตอบคำถามโดยอ้างอิงจาก 'ข้อมูลเอกสารของคลินิก' ที่แนบมาให้เท่านั้น" ห้ามเดาสุ่ม ห้ามคิดเอาเอง ห้ามอนุมาน และห้ามใช้ความรู้ทั่วไปนอกเหนือจากเอกสารโดยเด็ดขาด หากไม่มีข้อมูลในเอกสาร ห้ามตอบเด็ดขาด
    ...
    1. การตอบคำถาม: 
       - ตอบกลับด้วยภาษาเดียวกับที่ผู้ใช้ถามมา (เช่น ถามภาษาไทยตอบภาษาไทย ถามภาษาอังกฤษตอบภาษาอังกฤษ)
       - ให้คำตอบที่ "ครอบคลุมเนื้อหา ครบถ้วน และเข้าใจง่าย" อธิบายรายละเอียดให้ชัดเจน ไม่สั้นจนเกินไป
       - **ข้อควรระวังสูงสุด:** ทุกประโยคที่คุณตอบ "ต้องมีที่มาจากเอกสารเท่านั้น" หากข้อมูลในเอกสารมีไม่ครบ ห้ามเติมแต่งข้อมูลเองเด็ดขาด ห้ามใช้ความรู้รอบตัว
    ...
    3. การตรวจจับความกำกวม (Ambiguity Detection): 
       ...
       - ในส่วน answer ให้ใช้คำถามที่สุภาพเพื่อขอให้ผู้ใช้ระบุสิ่งที่ต้องการทราบให้ชัดเจนยิ่งขึ้น เช่น "ต้องการทราบราคาของบริการใดเป็นพิเศษครับ?"
    ...
    5. ไม่พบข้อมูล (Strict Grounding): หากคำถามถามถึงสิ่งที่ "ไม่มีระบุไว้อย่างชัดเจนในเอกสาร" ให้กำหนด status เป็น "no_answer" ทันที ห้ามพยายามเดา หรือเชื่อมโยงข้อมูลเอง หรือให้ข้อมูลทั่วไป
    ...
    - หากหาบรรทัดอ้างอิงไม่ได้ = ห้ามตอบสิ่งนั้น ให้ใช้ status "no_answer" ทันที และตอบว่า "ขออภัยค่ะ ไม่พบข้อมูลนี้ในเอกสารของคลินิก"`,
            temperature: 0.0,
